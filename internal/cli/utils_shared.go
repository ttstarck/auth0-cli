...
func BuildOauthTokenParams(clientID, clientSecret, audience string, scopes []string, organization string) url.Values {
	q := url.Values{
		"audience":      {audience},
		"client_id":     {clientID},
...
func runClientCredentialsFlow(
	...
	payload := BuildOauthTokenParams(client.GetClientID(), client.GetClientSecret(), audience, scopes, organization)
